...
### Option: `runner_labels` (optional)

Custom labels for the runner (comma-separated). Labels allow you to target specific runners in your GitHub Actions workflows.

If not set, the runner will use default labels: `self-hosted`, `Linux`, `X64` (or architecture-specific like `ARM`, `ARM64`).

**Important**: When you specify custom labels, they **replace** the default labels. If you want to keep the default labels, include them explicitly.

Examples:
- `"production"` - Single custom label (replaces defaults)
- `"self-hosted,Linux,X64,production"` - Custom label with defaults preserved
- `"fast,docker,custom"` - Multiple custom labels

Default: Empty (uses GitHub defaults: self-hosted, Linux, architecture)
...
- ✅ Configurable runner labels for workflow targeting (v1.4.0+)
